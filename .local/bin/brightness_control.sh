...
if command -v polybar-msg &> /dev/null
then
  polybar-msg hook screen_backlight 1
fi
sb=$(xbacklight);
sb=${sb%.*};
...
then
  msgId="991049"
  dunstify -u low -r "$msgId" -i $HOME/.icons/Vimix-dark/22/actions/settings.svg "Screen Brightness ${sb}" "$(tr '[:lower:]' '[:upper:]' <<< ${1:0:1})${1:1}reased by ${2}"
else
  notify-send -i $HOME/.icons/Vimix-dark/22/actions/settings.svg "Screen Brightness" "$(tr '[:lower:]' '[:upper:]' <<< ${1:0:1})${1:1}reased by ${2}"
fi
