...
import XMonad.Layout.Spacing ( toggleSmartSpacing, toggleScreenSpacingEnabled, toggleWindowSpacingEnabled )
...
addEWMHFullscreen   = do
   -- let xSpec = ["_NET_WM_STATE", "_NET_WM_STATE_FULLSCREEN", "_NET_WM_STATE_ABOVE","_NET_WM_STATE_STICKY","_NET_WM_STATE_DEMANDS_ATTENTION", "_NET_WM_FULLSCREEN_MONITORS"]
   -- let xSupport :: [X Atom] == []
   -- for(i = 0, i< xSpec.length; i++){ 
   --     xSupport ++ getAtom xSpec[i]
   -- }
   -- mapM_ addNETSupported xSupport 
...
windowRules = composeAll . concat $
    ...
    where
                ...
                myCFloats = ["alacritty-float", "MPlayer", "mpv","alacritty-cava",
                             ...
                             "Kvantum Manager", "qt5ct", "VirtualBox Manager",
                             "qemu", "Qemu-system-x86_64" ,"Lxappearance",
                             "Nitrogen", "Arandr", "Xfce4-power-manager-settings", 
                             "Nm-connection-editor","scrcpy"]
...
myManageHook =  windowRules <+> manageApps <+> manageSpawn <+> manageScratchpads <+>   raiseAllNotificationsHook
 where
  ...
  sendToBottom window = withDisplay $ \_display ->
      io $ lowerWindow _display window

  raiseWindow' :: Window -> X ()
  raiseWindow' window = withDisplay $ \_display ->
      io $ raiseWindow _display window

  allWindowsByType :: Query Bool -> X [Window]
  allWindowsByType _query = withDisplay $ \_display -> do
      (_, _, _windows) <- asks theRoot >>= io . queryTree _display
      filterM (runQuery _query) _windows
...
instance UrgencyHook LibNotifyUrgencyHook where
  urgencyHook LibNotifyUrgencyHook w = do
    ...
    traverse_ (\i -> safeSpawn "notify-send" [show _name, "workspace " ++ i]) maybeIdx
...
main' :: D.Client -> IO ()
main' dbus = xmonad . ewmh . docks . dynProjects . keybindings . _urgencyHook$ fullscreenSupport $ def {
  terminal           = myTerminal,
  ...
  where
    ...
    _logHook     = myPolybarLogHook dbus
    keybindings = addDescrKeys' ((myModMask, xK_F1), showKeybindings) myKeys
    _urgencyHook = withUrgencyHook LibNotifyUrgencyHook
