...
local interfaces = {
	...
	lan_interface = " enp7s0f4u1" --1enp2s0",
}
...
local return_button = function()
	...
	local check_internet_health = function(interface, callback)
		local command = [=[
	status_ping=0

	packets="$(ping -q -w2 -c2 1.1.1.1 -I ]=] .. interface .. [=[ | grep -o "100% packet loss")"
	if [ ! -z "${packets}" ];
	then
		status_ping=0
	else
	...
	local update_wireless = function()
		...
		local update_wireless_icon = function(strength)
			check_internet_health(interfaces.wlan_interface, function(stdout)
	...
	local update_wired = function()
		...
		check_internet_health(interfaces.lan_interface, function(stdout)
	...
	local check_network_mode = function()
		awful.spawn.easy_async_with_shell([=[
			...
			check_network_directory() {
				...
				then
					wireless_state="$(cat "${net}${wireless}/carrier")"
				fi
				if [ -n "${wired}" ] && [ -d "${net}${wired}" ]; then
					wired_state="$(cat "${net}${wired}/carrier")"
				fi
				check_network_state
			}
