...
if [[ -f "$DIR/../conf/rofi-network-manager.conf" ]]; then
	source "$DIR/../conf/rofi-network-manager.conf"
fi
...
function rofi_menu() {
	if [[ $(nmcli device | awk '$2=="wifi" {print $1}' | wc -l) -ne "1" ]]; then
		...
		-theme $DIR/../styles/networkmanager.rasi \
		-a "0" -lines "$LINES" -width -"$WIDTH" \
		-p "${WIRELESS_INTERFACES_PRODUCT[WLAN_INT]}[${WIRELESS_INTERFACES[WLAN_INT]}] - $(curl -s https://ipinfo.io/ip)")
	else
		SELECTION=$(echo -e "$WIFI_LIST\n~Scan\n~Manual\n$WIFI_SWITCH\n$WIRE_SWITCH\n~Status\n~Restart Network" | uniq -u | \
		rofi -dmenu -location "$LOCATION" -yoffset "$Y_AXIS" -xoffset "$X_AXIS" -font "$FONT" \
		-theme $DIR/../styles/networkmanager.rasi \
...
function change_wireless_interface() {
	if [[ $(nmcli device | awk '$2=="wifi" {print $1}' | wc -l) -eq "2" ]]; then
		...
		rofi -dmenu -location "$LOCATION" -yoffset "$Y_AXIS" -xoffset "$X_AXIS" -theme $DIR/../styles/networkmanager.rasi -font "$FONT"\
...
function ssid_manual() {
	...
	rofi -dmenu -location "$LOCATION" -yoffset "$Y_AXIS" -xoffset "$X_AXIS" -font "$FONT" \
	-a "0" -lines 1 -width 18 -theme $DIR/../styles/networkmanager.rasi \
	-p ">_")
	echo $SSID
	if [[ ! -z $SSID ]]; then
...
function status() {
	...
	echo -e "$ETH_STATUS\n${WLAN_STATUS[@]}\n"| \
	rofi -dmenu -location "$LOCATION" -yoffset "$Y_AXIS" -xoffset "$X_AXIS" -font "$FONT" \
	-theme $DIR/../styles/networkmanager.rasi -lines "$LINES" -width -"$WIDTH" \
	-p "Status"
}
