...
function change_wireless_interface() {
	if [[ $(nmcli device | awk '$2=="wifi" {print $1}' | wc -l) -eq "2" ]]; then
		...
		rofi -dmenu -location "$LOCATION" -yoffset "$Y_AXIS" -xoffset "$X_AXIS" -theme $DIR/../rofi/networkmanager.rasi -font "$FONT"\
...
function ssid_manual() {
	...
	rofi -dmenu -location "$LOCATION" -yoffset "$Y_AXIS" -xoffset "$X_AXIS" -font "$FONT" \
	-a "0" -lines 1 -width 18 -theme $DIR/../rofi/networkmanager.rasi \
	-p ">_")
	echo $SSID
	if [[ ! -z $SSID ]]; then
...
function status() {
	...
	echo -e "$ETH_STATUS\n${WLAN_STATUS[@]}\n"| \
	rofi -dmenu -location "$LOCATION" -yoffset "$Y_AXIS" -xoffset "$X_AXIS" -font "$FONT" \
	-theme $DIR/../rofi/networkmanager.rasi -lines "$LINES" -width -"$WIDTH" \
	-p "Status"
}
